// Origin: https://github.com/choonkeat/elm-element-navigation/pull/1
// This adds ports and node manipulation functions for Vendor.Browser.Elm
...
export function generateSpaFlags() {
  return {
...
export function extBrowserSetup(app, spaFlags, document, ele) {
    function isChild(node) {
        if (node === ele) return true;
        return (node && isChild(node.parentNode));
    }
    function getNearestAnchorTag (node) {
      return (
        node.href
          ? node
          : node.parentNode && getNearestAnchorTag(node.parentNode)
      )
    }
    ele.addEventListener("click", function(event) {
        if (event.metaKey || event.ctrlKey) return;
        const anchorTag = event.target && getNearestAnchorTag(event.target)
        if (! anchorTag) return;
    ...
    function badNode(n) {
        return (
            ...
            (n.className && (typeof n.className == typeof String) && n.className.match(/cvox_indicator_container/))
        );
    }
